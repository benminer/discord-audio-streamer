...
docker run -d --name discord-audio-streamer \
  ...
  -e YOUTUBE_PLAYLIST_LIMIT=${YOUTUBE_PLAYLIST_LIMIT:-15} \
  -e SPOTIFY_CLIENT_ID=$SPOTIFY_CLIENT_ID \
  -e SPOTIFY_CLIENT_SECRET=$SPOTIFY_CLIENT_SECRET \
  -e SPOTIFY_ENABLED=${SPOTIFY_ENABLED:-true} \
  -e SPOTIFY_PLAYLIST_LIMIT=${SPOTIFY_PLAYLIST_LIMIT:-10} \
  -e GEMINI_API_KEY=$GEMINI_API_KEY \
  -e GEMINI_ENABLED=$GEMINI_ENABLED \
  -e GEMINI_MODEL=$GEMINI_MODEL \
  -e GEMINI_TTS_MODEL=$GEMINI_TTS_MODEL \
  -e TTS_PROVIDER=${TTS_PROVIDER:-gemini} \
  -e GROK_TTS_API_KEY=$GROK_TTS_API_KEY \
  -e GROK_TTS_SPEED=${GROK_TTS_SPEED:-1.0} \
  -e DEEZER_ENABLED=${DEEZER_ENABLED:-true} \
  -e DEEZER_BPM_MATCHING=${DEEZER_BPM_MATCHING:-true} \
  -e IDLE_TIMEOUT_MINUTES=${IDLE_TIMEOUT_MINUTES:-20} \
  -e AUDIO_BITRATE=${AUDIO_BITRATE:-128000} \
