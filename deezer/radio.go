...
func GetArtistRadio(ctx context.Context, artistID int) ([]Track, error) {
	...
	if err != nil {
		...
		sentry.CaptureException(fmt.Errorf("deezer: get artist radio for %d: %w", artistID, err))
	...
	if err := json.Unmarshal(body, &results); err != nil {
		span.Status = sentry.SpanStatusInternalError
		sentry.CaptureException(fmt.Errorf("deezer: decode artist radio for %d: %w", artistID, err))
...
func GetGenreStations(ctx context.Context) ([]Genre, error) {
	...
	if err != nil {
		span.Status = sentry.SpanStatusInternalError
		sentry.CaptureException(fmt.Errorf("deezer: get genre stations: %w", err))
	...
	if err := json.Unmarshal(body, &results); err != nil {
		span.Status = sentry.SpanStatusInternalError
		sentry.CaptureException(fmt.Errorf("deezer: decode genre stations: %w", err))
...
func GetStationTracks(ctx context.Context, stationID int) ([]Track, error) {
	...
	if err != nil {
		span.Status = sentry.SpanStatusInternalError
		sentry.CaptureException(fmt.Errorf("deezer: get station tracks for %d: %w", stationID, err))
	...
	if err := json.Unmarshal(body, &results); err != nil {
		span.Status = sentry.SpanStatusInternalError
		sentry.CaptureException(fmt.Errorf("deezer: decode station tracks for %d: %w", stationID, err))
